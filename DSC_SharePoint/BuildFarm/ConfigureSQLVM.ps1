configuration ConfigureSQLVM
{
    ...
    Node localhost
    {
		...
		# Grant spfarm user local Administrator group membership
        Group Administrators
        {
            GroupName = "Administrators"
            MembersToInclude =  @("$DomainNetbiosName\$($SPSetupCreds.UserName)", "$DomainNetbiosName\$($SqlSvcCreds.UserName)" )
            Ensure = "Present"		
			DependsOn = @("[ADUser]CreateSPSetupAccount","[ADUser]CreateSqlSvcAccount")		
			PsDscRunAsCredential = $DomainAdminCredsQualified			
        }
        ...
        {			
            ...
            SourcePath = "\\vmware-host\Shared Folders\Media_Installer\SQL17Binaries"
            Features = "SQLENGINE"
            InstallSharedDir = "C:\Program Files\Microsoft SQL Server"
            SQLSysAdminAccounts = $SqlSvcCreds.UserName
            SQLSvcAccount = $SqlSvcCreds
            AgtSvcAccount = $SqlSvcCreds
            PSDscRunAsCredential = $DomainAdminCredsQualified
			ProductKey = "TDKQD-PKV44-PJT4N-TCJG2-3YJ6B"
            DependsOn = @("[PendingReboot]RebootOnSignalFromJoinDomain","[WindowsFeature]DotNet35","[Group]Administrators")
        }
        ...
        SqlLogin AddDomainAdminLogin
        {
            ...
            DependsOn    = "[PendingReboot]RebootOnSignalFromJoinDomain","[Script]EnsureSQLServiceStarted"
        }
        ...
        SqlLogin AddSPSetupLogin
        {
            ...
            InstanceName = "MSSQLSERVER"
            LoginType    = "WindowsUser"
            DependsOn    = "[ADUser]CreateSPSetupAccount","[Script]EnsureSQLServiceStarted"
        }
        ...
        SqlRole GrantSQLRoleSysadmin
        {
            ...
            InstanceName     = "MSSQLSERVER"
            Ensure           = "Present"
            DependsOn        = "[SqlLogin]AddDomainAdminLogin","[Script]EnsureSQLServiceStarted"
        }
        ...
        SqlRole GrantSQLRoleSecurityAdmin
        {
            ...
            InstanceName     = "MSSQLSERVER"
            Ensure           = "Present"
            DependsOn        = "[SqlLogin]AddSPSetupLogin","[Script]EnsureSQLServiceStarted"
        }
        ...
        SqlDatabaseUser AddSPSetupUserToTempdb
        {
            ...
            DependsOn            = "[SqlLogin]AddSPSetupLogin","[Script]EnsureSQLServiceStarted","[Script]EnsureSQLServiceStarted"
        }
        ...
        SqlDatabasePermission GrantPermissionssToTempdb
        {
            ...
            DependsOn            = "[SqlDatabaseUser]AddSPSetupUserToTempdb","[Script]EnsureSQLServiceStarted"
        }
        ...
        Firewall AddDatabaseEngineFirewallRule
        {
            ...
            LocalPort   = "1433"
            Ensure      = "Present"
			DependsOn            = "[Script]EnsureSQLServiceStarted"
        }
    }
}
...
$password = ConvertTo-SecureString -String "abcde12345-" -AsPlainText -Force
$DomainAdminCreds = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList "Administrator", $password
$SqlSvcCreds = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList "sqlsvc", $password
$SPSetupCreds = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList "spsetup", $password
$DNSServerIP = "192.168.73.4"
$DomainFQDN = "contoso.local"
